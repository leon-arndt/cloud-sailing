...
struct FAkPS4AdvancedInitializationSettings : public FAkAdvancedInitializationSettingsWithMultiCoreRendering
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "ACP batch buffer size used for ATRAC9 decoding."))
	uint32 ACPBatchBufferSize;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Use low latency mode for ATRAC9 (default is false). If true, decoding jobs are submitted at the beginning of the Wwise update and it will be necessary to wait for the result."))
