...
struct FAkMainOutputSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "The name of a custom audio device to be used. Custom audio devices are defined in the Audio Device Shareset section of the Wwise project. Leave this empty to output normally through the default audio device."))
	FString AudioDeviceShareset;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Device specific identifier, when multiple devices of the same type are possible. If only one device is possible, leave to 0."))
	uint32 DeviceID = AK_INVALID_UNIQUE_ID;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Rule for 3D panning of signals routed to a stereo bus. In \"Speakers\" mode, the angle of the front loudspeakers is used. In \"Headphones\" mode, the speaker angles are superseded with constant power panning between two virtual microphones spaced 180 degrees apart."))
	EAkPanningRule PanningRule = EAkPanningRule::Speakers;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "A code that completes the identification of channels by uChannelMask. Anonymous: Channel mask == 0 and channels; Standard: Channels must be identified with standard defines in AkSpeakerConfigs; Ambisonic: Channel mask == 0 and channels follow standard ambisonic order."))
	EAkChannelConfigType ChannelConfigType = EAkChannelConfigType::Anonymous;

	UPROPERTY(EditAnywhere, meta = (Bitmask, BitmaskEnum = EAkChannelMask, ToolTip = "A bit field, whose channel identifiers depend on AkChannelConfigType (up to 20)."))
	uint32 ChannelMask = 0;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "The number of channels, identified (deduced from channel mask) or anonymous (set directly)."))
...
struct FAkSpatialAudioSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Desired spatial audio memory pool size."))
	uint32 SpatialAudioPoolSize = 4 * 1024 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Maximum number of portals that sound can propagate through.", ClampMin = "0", ClampMax = "8"))
	...
	UPROPERTY(EditAnywhere, meta = (Bitmask, BitmaskEnum = EAkDiffractionFlags, ToolTip = "Determines whether diffraction values for sound passing through portals will be calculated, and how to apply those calculations to Wwise parameters."))
	uint32 DiffractionFlags = AkDiffractionFlags::DefaultDiffractionFlags;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Distance (in game units) that an emitter or listener has to move to trigger a recalculation of reflections/diffraction. Larger values can reduce the CPU load at the cost of reduced accuracy.", ClampMin = "0"))
...
struct FAkAdvancedSpatialAudioSettings
{
	...
	UPROPERTY(EditAnywhere, AdvancedDisplay, meta = (ToolTip = "Multiplier that is applied to the distance attenuation of diffracted sounds (sounds that are in the 'shadow region') to simulate the phenomenon where by diffracted sound waves decay faster than incident sound waves."))
	float DiffractionShadowAttenuationFactor = 2.0f;

	UPROPERTY(EditAnywhere, AdvancedDisplay, meta = (ToolTip = "Interpolation angle, in degrees, over which the \"Diffraction Shadow Attenuation Factor\" is applied."))
...
struct FAkCommunicationSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Size of the communication pool."))
	uint32 PoolSize = 256 * 1024;

	enum { DefaultDiscoveryBroadcastPort = 24024, };
	UPROPERTY(EditAnywhere, meta = (ToolTip = "The port where the authoring application broadcasts \"Game Discovery\" requests to discover games running on the network. Default value: 24024. (Cannot be set to 0)."))
	uint16 DiscoveryBroadcastPort = DefaultDiscoveryBroadcastPort;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "The \"command\" channel port. Set to 0 to request a dynamic/ephemeral port."))
	uint16 CommandPort = 0;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "The \"notification\" channel port. Set to 0 to request a dynamic/ephemeral port."))
	uint16 NotificationPort = 0;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "The name used to identify this game within the authoring application. Leave empty to use FApp::GetProjectName()."))
	FString NetworkName;
...
struct FAkCommunicationSettingsWithSystemInitialization : public FAkCommunicationSettings
{
	...
	UPROPERTY(Config, EditAnywhere, meta = (ToolTip = "Indicates whether the communication system should be initialized. Some consoles have critical requirements for initialization of their communications system. Set to false only if your game already uses sockets before sound engine initialization."))
...
struct FAkCommonInitializationSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Maximum number of memory pools. A memory pool is required for each loaded bank."))
	uint32 MaximumNumberOfMemoryPools = 256;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Maximum number of automation paths for positioning sounds."))
	uint32 MaximumNumberOfPositioningPaths = 255;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Size of the default memory pool."))
	uint32 DefaultPoolSize = 128 * 1024 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "The percentage of occupied memory where the sound engine should enter in Low memory Mode.", ClampMin = "0.0", ClampMax = "1.0"))
	float MemoryCutoffThreshold = 1.0f;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Size of the command queue."))
	uint32 CommandQueueSize = 256 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Number of samples per audio frame (256, 512, 1024, or 2048)."))
	uint32 SamplesPerFrame = 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Platform-independent initialization settings of output devices."))
	FAkMainOutputSettings MainOutputSettings;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Multiplication factor for all streaming look-ahead heuristic values.", ClampMin = "0.0", ClampMax = "1.0"))
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Lower Engine default memory pool size."))
	uint32 LowerEnginePoolSize = 16 * 1024 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "The percentage of occupied memory where the sound engine should enter in Low memory mode.", ClampMin = "0.0", ClampMax = "1.0"))
	float LowerEngineMemoryCutoffThreshold = 1.0f;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Number of refill buffers in voice buffer. Set to 2 for double-buffered, defaults to 4."))
...
struct FAkAdvancedInitializationSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Size of memory pool for I/O (for automatic streams). It is passed directly to AK::MemoryMgr::CreatePool(), after having been rounded down to a multiple of uGranularity."))
	uint32 IO_MemorySize = 2 * 1024 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Targeted automatic stream buffer length (ms). When a stream reaches that buffering, it stops being scheduled for I/O except if the scheduler is idle."))
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Maximum number of bytes that can be \"pinned\" using AK::SoundEngine::PinEventInStreamCache() or AK::IAkStreamMgr::PinFileInCache()."))
	uint32 MaximumPinnedBytesInCache = (uint32)-1;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Memory pool where data allocated by AK::SoundEngine::PrepareEvent() and AK::SoundEngine::PrepareGameSyncs() will be done."))
	int32 PrepareEventMemoryPoolID = AK_INVALID_POOL_ID;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Set to true to enable AK::SoundEngine::PrepareGameSync usage."))
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Size of the monitoring pool. This parameter is not used in Release build."))
	uint32 MonitorPoolSize = 256 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Size of the monitoring queue pool. This parameter is not used in Release build."))
	uint32 MonitorQueuePoolSize = 64 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Amount of time to wait for hardware devices to trigger an audio interrupt. If there is no interrupt after that time, the sound engine will revert to silent mode and continue operating until the hardware finally comes back."))
...
struct FAkAdvancedInitializationSettingsWithMultiCoreRendering : public FAkAdvancedInitializationSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Allow to distribute SoundEngine processing tasks across multiple threads. Requires Editor restart."))
