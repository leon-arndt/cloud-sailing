...
struct FAkXBoxOneAdvancedInitializationSettings : public FAkAdvancedInitializationSettingsWithMultiCoreRendering
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "Maximum number of hardware-accelerated XMA voices used at run-time. Default is 128 voices."))
	uint16 MaximumNumberOfXMAVoices = 128;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "Use low latency mode for hardware XMA decoding (default is false). If true, decoding jobs are submitted at the beginning of the Wwise update and it will be necessary to wait for the result."))
...
struct FAkXBoxOneApuHeapInitializationSettings
{
	...
	UPROPERTY(EditAnywhere, meta = (ToolTip = "APU heap cached size sent to the \"ApuCreateHeap()\" function."))
	uint32 CachedSize = 64 * 1024 * 1024;

	UPROPERTY(EditAnywhere, meta = (ToolTip = "APU heap non-cached size sent to the \"ApuCreateHeap()\" function."))
